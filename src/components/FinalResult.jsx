...
const FinalResult = ({ result }) => {
 ...
 if (
  ...
  return (
    <div className='mt-3 space-y-10 bg-white'>
      <div className='flex flex-col md:flex-row md:items-center md:justify-between gap-4'>
        ...
        <button onClick={()=>{
          setquickrevision(!quickrevision)
        }} className={`px-4 py-2 rounded-lg text-sm font-medium transition ${quickrevision?"bg-green-600 text-white":"bg-green-100 text-green-700 hover:bg-green-200"}`}>{quickrevision ? "Exit Revision mode":"quick Revision(5 min)"}</button>
        <button className='px-4 py-2 rounded-lg text-sm font-medium bg-indigo-600 text-white hover:bg-indigo-700' onClick={ () => {
         generatePDF(result)}}> ⬇️ Download Pdf</button>
        </div>
      </div>
...
{result.notes}
      ...
      {quickrevision && <section className='rounded-xl bg-linear-to-r from-green-100 to-green-50 border border-green-200 p-6'>
...
<SectionHeader icon='📊
' title='Diagram' color='cyan' />
<MermaidSetup diagram={result.diagram?.data} />
<p className='mt-3 text-xs text-gray-500 italic flex items-center gap-2'>
  ...
  you can save it by taking a screenshot.
    ...
    { result.charts?.length >0 && <section>
      <SectionHeader icon="📈" title="Visual Charts" color="indigo"></SectionHeader>
